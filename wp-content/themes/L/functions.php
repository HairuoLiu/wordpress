...
function cx_foot(){
	...
	$output .= '<p class="footer_menus">'.strip_tags(wp_nav_menu( $menus ), '<a>' ).'</p><p> 版权所有 Copyright © by <a href="http://pico.org.ohio-state.edu"> WordPress </a>';
	$output .= date('Y');		
	$output .= ' '.$site_title.'<span> .AllRights Reserved';	
	if(isset($_foot_ba_url) && $_foot_ba_url =='off'){
     $output .= '<a href="http://pico.org.ohio-state.edu" rel="nofollow" target="_blank">'.$_foot_ba.'</a>';
	} else {
	$output .= ' '.$_foot_ba;
	}	
	$output .= '</span></p><p> Made By <a href="http://pico.org.ohio-state.edu">HairuoLiu</a> ';
	$output .= baidu_tongji();
	$output .= '</p>';
	$output .= '</div>';
	echo $output;
